      ...
      >
        ...
        <div className="px-4 pt-4 pb-3 border-b border-gray-100 flex items-start justify-between gap-2">
          <div className="flex-1 min-w-0 space-y-1.5">
            {/* Dataset name as blue pill */}
            <span
              className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold border truncate max-w-[240px]"
            ...
            <div className="flex items-center gap-2">
              <span className="font-mono text-[10px] text-gray-400 tabular-nums border border-gray-200 rounded px-1.5 py-0.5 bg-gray-50">
                Dataset ID {fmtId(dataset.id)}
              </span>
              <span className="text-[11px] text-gray-400">{fmtRows(dataset.row_count)}</span>
            </div>
          </div>
          <button onClick={onCancel} className="text-gray-400 hover:text-gray-600 transition flex-shrink-0 mt-0.5 p-0.5 rounded">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
          ...
          <div className="space-y-1.5">
            {[baselineScenario, ...namedScenarios].filter(Boolean).map((sc) => (
              ...
              >
                ...
                {/* Scenario ID badge — right-aligned */}
        ...
        <div className="px-4 pb-4 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 text-white rounded-xl text-xs font-semibold shadow-sm transition disabled:opacity-60"
            style={{ background: NAVY }}
            onMouseEnter={(e) => { if (!loading && selectedId != null) e.currentTarget.style.background = "#0a3366"; }}
