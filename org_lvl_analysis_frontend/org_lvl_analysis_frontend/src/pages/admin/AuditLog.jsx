...
const ACTION_COLORS = {
  ...
  "project.assign": "bg-brand-50 text-brand-700",
    ...
    <div>
      ...
      <div className="bg-white rounded-md shadow-sm border border-gray-200 p-3 mb-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          {hasFilters && (
            <button onClick={clearFilters} className="ml-auto text-xs text-brand-600 hover:underline">Clear all</button>
          )}
        </div>
        ...
        <>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-[#01244a] border-b border-[#08304a]">
                  <th className="text-left px-4 py-2 text-xs font-semibold text-white uppercase tracking-wider w-56">Time (IST)</th>
                  <th className="text-left px-4 py-2 text-xs font-semibold text-white uppercase tracking-wider w-28">User</th>
                  <th className="text-left px-4 py-2 text-xs font-semibold text-white uppercase tracking-wider w-36">Action</th>
                  <th className="text-left px-4 py-2 text-xs font-semibold text-white uppercase tracking-wider">Details</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {entries.map((e) => (
                  <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-2 text-xs text-gray-500 font-mono whitespace-nowrap" title={e.created_at + " UTC"}>
                      {formatIST(e.created_at)}
                    </td>
                    <td className="px-4 py-2 text-sm text-gray-700 font-medium">{e.username || "—"}</td>
                    <td className="px-4 py-2">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                        ACTION_COLORS[e.action] || "bg-gray-100 text-gray-600"
                      }`}>{e.action}</span>
                    </td>
                    <td className="px-4 py-2 text-xs text-gray-500 truncate max-w-md" title={formatDetails(e) || ""}>
                      {formatDetails(e) || "—"}
                    </td>
                  </tr>
                ))}
          ...
          <div className="flex items-center justify-between mt-4">
            <button onClick={() => load(Math.max(0, offset - PAGE_SIZE))} disabled={offset === 0}
              className="px-3 py-1.5 bg-white border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
              Previous
            </button>
