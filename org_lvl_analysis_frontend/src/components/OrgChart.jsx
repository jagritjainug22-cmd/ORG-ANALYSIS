...
export default function OrgChart({
  ...
  const { hidden, searchMatches } = useMemo(() => {
    ...
    const matchesEmp = (r) => {
      ...
      if (jobTitleFilter) {
        const title = jobTitleCol ? String(r[jobTitleCol] ?? "") : "";
        if (title !== jobTitleFilter) return false;
      }
      if (functionFilter) {
        const func = funcCol ? String(r[funcCol] ?? "") : "";
        if (func !== functionFilter) return false;
      }
      if (term) {
        const haystack = [
          r[empCol],
  ...
  // Unique job titles for the filter dropdown — mapped column only.
  const jobTitles = useMemo(() => {
    if (!records || !jobTitleCol) return [];
    const set = new Set();
    records.forEach((r) => {
      const t = r[jobTitleCol];
      if (t) set.add(String(t));
    });
    return Array.from(set).sort();
  }, [records, jobTitleCol]);

  // Unique functions for the filter dropdown — mapped column only.
  const functions = useMemo(() => {
    if (!records || !funcCol) return [];
    const set = new Set();
    records.forEach((r) => {
      const f = r[funcCol];
