...
function insightNumber(insights, key) {
  if (!insights || insights[key] == null || insights[key] === "") return null;
  const n = Number(insights[key]);
  return Number.isFinite(n) ? n : null;
}

/** Plotly inside a newly-opened modal often measures width as 0 / too small. Size the
 *  graph from the container after layout (and after the fade-in) so axes, bars, and
 *  legends fill the modal instead of clustering on the left. */
function ModalPlot({ data, layout, config, height = 280 }) {
  ...
  return (
    <div ref={boxRef} className="w-full min-w-0" style={{ height }}>
      {width > 40 && (
        <Plot
          data={data}
          layout={{ ...layout, autosize: false, width, height }}
          config={{ displayModeBar: false, responsive: false, ...config }}
...
export default function SpansLayers({
  ...
  const [toast, setToast] = useState(null); // { message, type } — auto-dismisses
  const [newTabKey, setNewTabKey] = useState(null); // key of most-recently created tab, for pulse animation
  ...
  // Baseline always reflects the current data/filters (threshold=0).
  // Scenario results are separate — they NEVER replace the baseline display.
  const baselineResult = baseline.result;
  const scenarioResult = activeScenario?.result || null;
  const hasSimulation = !!scenarioResult;
  // Threshold for the simulate input (not tied to active scenario)
  const threshold = pendingThreshold;

  const setThreshold = useCallback((val) => {
    setPendingThreshold(Number(val));
  }, []);

  // Sync the threshold input to the active scenario whenever the user switches tabs.
  // This ensures the input always reflects what's being viewed, not an unrelated pending value.
  useEffect(() => {
    if (activeScenario?.threshold != null && Number(activeScenario.threshold) > 0) {
      setPendingThreshold(Number(activeScenario.threshold));
    }
  ...
  const runScenarioSimulation = async (downloadMode = false) => {
    ...
    if (!downloadMode && scenarios.length >= MAX_SCENARIOS) {
      setError(`Maximum of ${MAX_SCENARIOS} scenarios. Delete one to simulate another.`);
      return;
    }
    ...
    try {
      ...
      if (!downloadMode) {
        // Always create a NEW scenario tab — no overwriting existing ones
        const newScenario = makeDraftScenario(scenarios.length + 1, t);
        const patched = { ...newScenario, threshold: t, result: res };
        const saved = await persistActiveScenario(patched);
        const key = scenarioKey(saved);
        setScenarios((prev) => [...prev, saved]);
        setActiveKey(key);
        setNewTabKey(key);
        setSelectedLayer(null);
        setViewMode("scenario");
        // Scroll to tab bar and notify user
        setTimeout(() => {
          tabBarRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
        }, 60);
        showToast(`New tab opened — Span ${t} simulation ready`, "success");
        // Remove pulse after animation completes
        setTimeout(() => setNewTabKey(null), 2000);
      }
    } catch (err) {
      console.error(err);
  ...
  const downloadSummary = () => {
    if (!baselineResult || !baselineResult.summary || baselineResult.summary.length === 0) {
      setError("No summary data available to download.");
      return;
    }

    try {
      const ws = XLSX.utils.json_to_sheet(baselineResult.summary);
  ...
  const downloadPPT = async () => {
    if (!baselineResult || !baselineResult.summary?.length) {
    ...
    try {
      ...
      const ins = baselineResult.insights || {};
  ...
  // Baseline display data (always visible, never overridden by scenarios)
  const baselineSummary = baselineResult?.summary || [];
  const baselineInsights = baselineResult?.insights;
  const spanDistribution = baselineInsights?.span_distribution || [];
  const baselineByFunction = baselineInsights?.by_function || [];
  const funcHighlights = baselineInsights?.function_highlights || {};
  const hasCostData = !!baselineInsights?.has_cost_data || !!flcCol;

  // Scenario-specific display data (only for the active scenario tab)
  const scenarioInsights = scenarioResult?.insights;
  const scenarioByFunction = scenarioInsights?.by_function || [];
  const scenarioRedesignPriority = scenarioInsights?.redesign_priority || [];

  // For baseline zone: summary table uses baselineSummary
  const summary = baselineSummary;
  const insights = baselineInsights;

  // Function table rows — scenario view sorts by opportunity, baseline shows raw
  const functionTableRows = useMemo(() => {
    if (hasSimulation && scenarioByFunction.length) {
      return [...scenarioByFunction].sort(
        (a, b) =>
    ...
    if (!baselineByFunction.length) return [];
    return baselineByFunction;
  }, [baselineByFunction, scenarioByFunction, hasSimulation]);

  // Scale for the inline "vs org avg" mini-bar in the function benchmark table.
  const byFunction = hasSimulation ? scenarioByFunction : baselineByFunction;
  const maxFuncAvgSpan = useMemo(
    () => Math.max(...(hasSimulation ? scenarioByFunction : baselineByFunction).map((r) => r.avg_span || 0), 1),
    [baselineByFunction, scenarioByFunction, hasSimulation]
  );
  ...
  const redesignPriority = scenarioRedesignPriority;
  const redesignChart = useMemo(() => {
    if (!scenarioRedesignPriority.length) return null;
    const top = scenarioRedesignPriority.slice(0, 8);
  ...
  }, [scenarioRedesignPriority, hasCostData]);
  ...
  // One mini-chart per KPI so mixed scales (avg span vs headcount vs FTE) each get a full axis.
  const compareKpiCharts = useMemo(() => {
    if (ranScenarios.length < 2) return null;
    const names = ["Baseline", ...ranScenarios.map((s) => s.name || `Span ${s.threshold}`)];
    const metricDefs = [
    ...
    return metricDefs.map((m) => {
      const y = [
        m.key === "avg_span" ? insightNumber(baselineInsights, "avg_span") : null,
        ...ranScenarios.map((s) => insightNumber(s.result?.insights, m.key)),
      ];
      const numeric = y.filter((v) => v != null);
      const maxY = Math.max(1, ...numeric);
      const labels = y.map((v) => (v == null ? "—" : String(v)));
      return {
        key: m.key,
        label: m.label,
        data: [{
          ...
          textfont: { size: 11, family: CHART_FONT, color: NAVY, weight: 700 },
          cliponaxis: false,
          hovertemplate: `<b>%{x}</b><br>${m.label}: %{text}<extra></extra>`,
        }],
        layout: {
          margin: { l: 36, r: 10, t: 28, b: 40 },
          paper_bgcolor: "white",
          plot_bgcolor: "white",
          font: { family: CHART_FONT, size: 11, color: "#334155" },
          bargap: 0.38,
          showlegend: false,
          xaxis: {
            tickfont: { size: 10, family: CHART_FONT, color: NAVY },
            fixedrange: true,
            automargin: true,
            tickangle: names.length > 3 ? -20 : 0,
          },
          yaxis: {
            gridcolor: "#eef2f7",
            zeroline: false,
  ...
  const compareFunctionChart = useMemo(() => {
    ...
    const traces = ranScenarios.map((s, i) => {
      const x = ranked.map((fn) => {
        if (!s.result?.insights) return null;
        const row = (s.result.insights.by_function || []).find((f) => f.function === fn);
        if (!row || row[metric] == null || row[metric] === "") return null;
      ...
      return {
        ...
        hovertemplate: `<b>%{y}</b><br>${s.name || `Span ${s.threshold}`}: %{x}<extra></extra>`,
      };
    });

    const allVals = traces.flatMap((t) => t.x).filter((v) => v != null && v > 0);
    if (!allVals.length) {
      return { empty: true, metric: compareHasCostData ? "cost" : "fte" };
    }

    const maxVal = Math.max(...allVals);
    const height = Math.max(280, 88 + ranked.length * 38);
    ...
    return {
      ...
      layout: {
        ...
        bargap: 0.22,
        bargroupgap: 0.12,
        margin: { l: 16, r: 20, t: 36, b: 52 },
        paper_bgcolor: "white",
        plot_bgcolor: "white",
        font: { family: CHART_FONT, size: 11, color: "#334155" },
        xaxis: {
          title: {
            text: compareHasCostData ? "Cost opportunity ($)" : "FTE opportunity",
            font: { size: 11, family: CHART_FONT, color: "#64748b" },
            standoff: 10,
          },
          gridcolor: "#eef2f7",
          zeroline: false,
        ...
        yaxis: {
          tickfont: { size: 11, family: CHART_FONT, color: NAVY },
          fixedrange: true,
          automargin: true,
        },
        legend: {
  ...
  return (
    <div className="p-4 space-y-4 overflow-auto h-full">
      {/* Toast notification — fixed bottom-right, auto-dismisses */}
      {toast && (
        <div
          className={`fixed bottom-6 right-6 z-[100] flex items-center gap-2.5 px-4 py-3 rounded-xl shadow-2xl border text-sm font-semibold transition-all duration-300 animate-fadeInUp ${
            toast.type === "success"
              ? "bg-[#01244a] text-white border-[#01244a]/30"
              : "bg-amber-50 text-amber-800 border-amber-200"
          }`}
        >
          {toast.type === "success" ? (
            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-teal-300 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">
            Baseline is always visible below. Set a target threshold and hit Simulate to create scenario tabs, then Compare.
            {datasetId ? " Scenarios save with this dataset." : ""}
          </p>
        </div>
      </div>

      {/* Tab bar — always visible; Baseline tab is permanent and cannot be deleted */}
      <div ref={tabBarRef} className="bg-white border border-brand-100 rounded-xl shadow-sm px-2 py-2">
        <div className="flex flex-wrap items-center gap-1.5">
          {/* Permanent Baseline tab */}
          <button
            type="button"
            onClick={() => {
              setActiveKey(null);
              ...
              setSelectedLayer(null);
              setError(null);
              setTimeout(() => tabBarRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 50);
            }}
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg border transition-all duration-150 ${
              viewMode === "scenario" && activeKey === null
                ? "bg-[#01244a] text-white border-[#01244a] shadow-sm"
                : "bg-white text-slate-700 border-slate-200 hover:border-[#01244a]/40 hover:bg-slate-50"
            }`}
            title="View baseline (current state)"
          >
          ...
          {/* Separator if there are scenario tabs */}
          {scenarios.length > 0 && (
          ...
          {scenarios.map((s) => {
            ...
            return (
              <div
                ...
                } ${isNew ? "ring-2 ring-brand-400 ring-offset-1 animate-pulse" : ""}`}
              >
                <button
                  type="button"
                  onClick={() => {
                    ...
                    setSelectedLayer(null);
                    setError(null);
                    if (s.threshold > 0) setPendingThreshold(Number(s.threshold));
                    setTimeout(() => tabBarRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 50);
                  }}
                ...
                >
                  ...
                  <span className={`ml-1.5 text-[10px] font-normal ${active ? "text-white/80" : "text-slate-500"}`}>
                    target {s.threshold}
                  </span>
                </button>
                <button
      ...
      {/* Compare modal — rendered as a fullscreen overlay */}
      {viewMode === "compare" && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm"
          onClick={(e) => { if (e.target === e.currentTarget) setViewMode("scenario"); }}
        >
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl mx-4 max-h-[90vh] flex flex-col overflow-hidden animate-fadeInUp">
            {/* Modal header */}
            <div className="px-5 py-3.5 border-b border-slate-100 bg-gradient-to-r from-[#01244a] to-[#01244a]/90 flex items-center justify-between flex-shrink-0">
              <div>
                <h4 className="text-sm font-semibold text-white font-display">Scenario comparison</h4>
                <p className="text-[11px] text-white/60 mt-0.5">Side-by-side metrics from each simulated threshold scenario</p>
              </div>
              <button
                type="button"
            ...
            <div className="overflow-y-auto flex-1">
              {compareKpiCharts && (
                <div className="p-4 border-b border-slate-100">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-3">KPIs at a glance</p>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {compareKpiCharts.map((chart) => (
                      <div key={chart.key} className="min-w-0 rounded-lg border border-slate-100 bg-slate-50/40 px-2 pt-2 pb-1">
                        <p className="text-[11px] font-semibold text-[#01244a] px-1" style={{ fontFamily: CHART_FONT }}>
                          {chart.label}
                        </p>
                        <ModalPlot
                          data={chart.data}
                ...
                <table className="w-full text-xs border-collapse">
                  <thead className="bg-[#01244a] text-white">
                    <tr>
                      ...
                      <th className="px-3 py-2.5 text-right font-semibold whitespace-nowrap bg-[#01244a]/80">
                        Baseline
                        <span className="block text-[10px] font-normal text-white/70">current state</span>
                      </th>
                      {ranScenarios.map((s) => (
                        <th key={scenarioKey(s)} className="px-3 py-2.5 text-right font-semibold whitespace-nowrap">
                          {s.name}
                          <span className="block text-[10px] font-normal text-white/70">target {s.threshold}</span>
                        </th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      { label: "Avg span", base: baselineInsights?.avg_span ?? "—", get: (s) => s.result?.insights?.avg_span ?? "—" },
                      { label: "Managers", base: baselineInsights?.total_managers ?? "—", get: (s) => s.result?.insights?.total_managers ?? "—" },
                      { label: "Mgmt depth", base: baselineInsights?.management_depth ?? "—", get: (s) => s.result?.insights?.management_depth ?? "—" },
                      { label: "1:1 managers", base: baselineInsights?.one_to_one_count ?? "—", get: (s) => s.result?.insights?.one_to_one_count ?? "—" },
                      { label: "Thin layers", base: baselineInsights?.thin_layer_count ?? "—", get: (s) => s.result?.insights?.thin_layer_count ?? "—" },
                      { label: "Below target", base: "—", get: (s) => s.result?.insights?.below_target_count ?? "—" },
                      { label: "High / Low span", base: "—", get: (s) => `${s.result?.high ?? "—"} / ${s.result?.low ?? "—"}` },
                      { label: "FTE opportunity", base: "—", get: (s) => s.result?.insights?.fte_opportunity ?? "—" },
                      { label: "Cost opportunity", base: "—", get: (s) => fmtCost(s.result?.insights?.cost_opportunity) },
                    ].map((row, idx) => {
                      const allDashes = ranScenarios.every((s) => row.get(s) === "—" || row.get(s) === "— / —");
                      return (
                        <tr key={row.label} className={idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}>
                          ...
                          <td className="px-3 py-2 text-right tabular-nums font-medium text-slate-500 border-b border-slate-100 bg-slate-50/30">
                            {row.base}
                          </td>
                          {ranScenarios.map((s) => (
                          ...
                          {allDashes && ranScenarios.length > 0 && (
                            <td className="px-3 py-2 border-b border-slate-100">
                              <span className="text-[10px] text-amber-500 whitespace-nowrap">⚠ Re-simulate</span>
                            </td>
                          )}
                        </tr>
                      );
              ...
              {ranScenarios.some((s) => !s.result?.insights) && (
                <div className="mx-4 mt-3 mb-1 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 flex items-start gap-2">
                  ...
                  <p className="text-[11px] text-amber-700 leading-relaxed">
                    One or more scenarios show "—" because their data is stale. Close this modal, delete those tabs, and re-simulate to refresh.
                  </p>
                </div>
              )}
              ...
              {ranScenarios.some((s) => s.result?.insights?.by_function?.length) && (
                <div className="border-t border-slate-100 p-4">
                  ...
                  {compareFunctionChart?.empty ? (
                    <div className="mb-4 rounded-lg border border-slate-100 bg-slate-50 px-3 py-6 text-center">
                      <p className="text-xs text-slate-500">
                        No {compareFunctionChart.metric === "cost" ? "cost" : "FTE"} opportunity to plot — every compared function is {compareFunctionChart.metric === "cost" ? "$0" : "0"} or unavailable.
                      </p>
                    </div>
                    ...
                    <div className="mb-4 min-w-0">
                      <ModalPlot
                        ...
                        height={compareFunctionChart.height}
                      />
                    </div>
                  ) : null}
            ...
            {/* Modal footer */}
            <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/50 flex justify-end flex-shrink-0">
              <button
                type="button"
                onClick={() => setViewMode("scenario")}
      ...
      {/* ═══ ZONE 1: Always-visible baseline content ═══ */}
      ...
      {summary.length > 0 && (
        <>
          ...
          {/* Span distribution + function highlights — always uses baseline data */}
          {insights && (spanDistChart || baselineByFunction.length > 0) && (
                  ...
                  {hasSimulation && (
                    <>
                      <div className="flex items-center gap-1.5 px-2.5 py-1 bg-[#01244A] rounded-md">
                        ...
                        <span className="text-sm font-bold text-white">{scenarioResult?.high ?? 0}</span>
                      </div>
                      <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white border border-[#01244A]/30 rounded-md">
                        ...
                        <span className="text-sm font-bold text-[#01244A]">{scenarioResult?.low ?? 0}</span>
                      </div>
                      <button
                        type="button"
                  ...
                  <>
                    ...
                    {!scenarioInsights && (
                      <div className="mt-3 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 flex items-start gap-2">
                        ...
                        <p className="text-[11px] text-amber-700 leading-relaxed">
                          High/Low counts are available but opportunity metrics could not be computed for this tab (stale or backend error).
                          Delete this tab and re-simulate to get FTE and cost data.
                          {scenarioResult?.insights_error ? ` Error: ${scenarioResult.insights_error}` : ""}
                        </p>
                      </div>
                    )}
                    {/* Warn: threshold ≤ 1 is trivial */}
                    {scenarioInsights && Number(activeScenario?.threshold) <= 1 && (
                      <div className="mt-3 bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 flex items-start gap-2">
                        ...
                        <p className="text-[11px] text-blue-700 leading-relaxed">
                          Target span {activeScenario.threshold} means every manager qualifies as "High span" — no one is below target. Try a threshold of 4–6 for a meaningful analysis.
                        </p>
                      </div>
                    )}
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
                      <div className="rounded-lg bg-white/80 border border-brand-100 px-3 py-2.5">
                        ...
                        <p className="text-xl font-extrabold text-[#01244a] mt-1 font-display">{activeScenario?.threshold ?? threshold}</p>
                        <p className="text-[10px] text-slate-400 mt-0.5">simulated threshold</p>
                      </div>
                      <div className="rounded-lg bg-white/80 border border-brand-100 px-3 py-2.5">
                        <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Below target</p>
                        <p className={`text-xl font-extrabold mt-1 font-display ${(scenarioInsights?.below_target_count ?? scenarioResult?.low ?? 0) > 0 ? "text-amber-700" : "text-slate-400"}`}>
                          {scenarioInsights != null
                            ? (scenarioInsights.below_target_count ?? 0)
                            : (scenarioResult?.low ?? "—")}
                        </p>
                        <p className="text-[10px] text-slate-400 mt-0.5">managers with span &lt; {activeScenario?.threshold}</p>
                      </div>
                      <div className="rounded-lg bg-white/80 border border-brand-100 px-3 py-2.5">
                        <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">FTE opportunity</p>
                        <p className={`text-xl font-extrabold mt-1 font-display ${scenarioInsights?.fte_opportunity > 0 ? "text-brand-700" : "text-slate-400"}`}>
                          {scenarioInsights != null ? (scenarioInsights.fte_opportunity ?? 0) : "—"}
                        </p>
                        <p className="text-[10px] text-slate-400 mt-0.5">
                          {scenarioInsights != null ? "potential FTE reduction" : "re-simulate to compute"}
                        </p>
                      </div>
                      <div className="rounded-lg bg-white/80 border border-brand-100 px-3 py-2.5">
                        <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Cost savings</p>
                        <p className={`text-xl font-extrabold mt-1 font-display ${scenarioInsights?.cost_opportunity > 0 ? "text-brand-700" : "text-slate-400"}`}>
                          {scenarioInsights != null && hasCostData
                            ? (scenarioInsights.cost_opportunity > 0 ? fmtCost(scenarioInsights.cost_opportunity) : "—")
                            : "—"}
                        </p>
                        <p className="text-[10px] text-slate-400 mt-0.5">
                          {scenarioInsights == null
                            ? "re-simulate to compute"
                            : hasCostData
                              ? "from below-target managers"
                              : "map FLC column for cost"}
                        </p>
                      </div>
                    </div>
                    {redesignChart && (
            ...
            <div className="mt-4">
              <div className="border-b border-gray-200 flex gap-0">
                {[
                  ...
                  { id: "below", label: "Below Target", count: scenarioInsights?.below_target_count || 0 },
                ...
                {analysisTab === "below" && (
                  <>
                    <div className="mb-4 flex flex-wrap gap-2">
                      {hasSimulation && scenarioInsights ? (
                        <>
                          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-brand-50 border border-brand-200 rounded-full text-xs font-semibold text-brand-700">
                            {scenarioInsights.below_target_count} manager{scenarioInsights.below_target_count !== 1 ? "s" : ""} below target span of {activeScenario?.threshold}
                          </span>
                          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-slate-200 rounded-full text-xs font-semibold text-slate-700">
                            FTE opp: {scenarioInsights.fte_opportunity}
                          </span>
                          {hasCostData && (
                            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 border border-amber-200 rounded-full text-xs font-semibold text-amber-800">
                              Cost opp: {fmtCost(scenarioInsights.cost_opportunity)}
                            </span>
                          )}
                        </>
                      ) : (
                        <span className="text-sm text-gray-500">Simulate a target span threshold to identify below-target managers.</span>
                      )}
                    </div>
                    {hasSimulation && scenarioInsights?.below_target?.length > 0 ? (
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                          <thead className="bg-[#01244a] text-white">
                            <tr>
                          ...
                          <tbody className="divide-y divide-gray-50">
                            {scenarioInsights.below_target.map((m) => (
